...
#To tokenize and to convert into lower case,removal of stop words.
...
def cosineScore(listA,listB,op,docid):
	...
	if op =='and':	
		...
		magB = math.sqrt(sum(counterB.get(k, 0)**2 for k in terms))
		if(dotprod != 0):
			print((dotprod / (magA * magB)),math.log(int(lines[docid])),(dotprod / (magA * magB)) + math.log(int(lines[docid])) )
			return ((dotprod / (magA * magB)) + math.log(int(lines[docid]))) 
		else:
			return 0
...
def cosrelevence(request):
	if request.method == "POST":
		...
		file_path = os.path.join(module_dir, 'documents')
		i=0
	...
	return render(request,'tfidf/Cossearchcontent.html',context={"content":string,"urllink":link})
...
def relevence(request):
	if request.method == "POST":
		...
		string = string.replace(link,"")
		return render(request,'tfidf/searchcontent.html',context={"content":string,"urllink":link})
	...
	if request.method == "POST" :
		...
		if len(scores)==0:
			...
			for id in top_docs:
				...
				rank+=1
		if request.POST["radio"] == "1":
			return render(request,'tfidf/searchresults.html', context= {"query":original,"num":len(scores),"results": result})
		elif request.POST["radio"] == "0":
			return render(request,'tfidf/Cossearchresults.html', context= {"query":original,"num":len(scores),"results": result})			
	return render(request,'tfidf/home.html')